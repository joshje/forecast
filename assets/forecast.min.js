...
get_forecast_ajax=function(a,d){message(wrap("p","Looking at the sky&hellip;")+wrap("h1","Loading",{"class":"loading"}));JSONP.get("http://free.worldweatheronline.com/feed/weather.ashx",{key:api_key,date:"tomorrow",cc:"no",q:a+","+d,format:"json"},function(b){b.data.error?forecast_error():(set_forecast_cache(b,a,d),forecast_success(b,a,d))});timeout=setTimeout(function(){forecast_error()},5E3)},forecast_success=function(a){clearTimeout(timeout);var d=forecast_lookup(a.data.weather[0].weatherCode);
d||(d=a.data.weather[0].weatherDesc[0].value);message(wrap("p","Tomorrow&rsquo;s forecast is")+wrap("h1",d))},forecast_error=function(){message(wrap("p","Don&rsquo;t panic, but we couldn&rsquo;t find any weather right now.")+wrap("p",wrap("a","Try again",{href:"#","class":"retry"})))},forecast_lookup=function(a){for(type in weather_types)for(c in weather_types[type])if(a==weather_types[type][c])return type};addEvent(document.getElementsByTagName("body")[0],"click",function(a){-1!==a.target.className.indexOf("retry")&&
(get_location(),a.preventDefault())});get_location()}else message(wrap("p","Drat. We can&rsquo;t find you.")+wrap("p",'You might need a <a href="http://www.google.com/chrome">modern browser</a>'));